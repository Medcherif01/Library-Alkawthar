...
mongoose.connect(process.env.MONGODB_URI)
  .then(() => console.log('Connexion à MongoDB réussie via Vercel.'))
...
// Routes API
app.get('/api/books', async (req, res) => { const books = await Book.find().sort({ title: 1 }); res.json(books); });
app.get('/api/loans', async (req, res) => { const loans = await Loan.find(); res.json(loans); });
app.post('/api/books/import', async (req, res) => {
    const booksToImport = req.body; let addedCount = 0, updatedCount = 0;
    for (const bookData of booksToImport) {
        if (!bookData.isbn || !bookData.title) continue;
        const existingBook = await Book.findOne({ isbn: bookData.isbn });
        if (existingBook) { existingBook.totalCopies += isNaN(bookData.totalCopies) ? 0 : bookData.totalCopies; await existingBook.save(); updatedCount++; } else { await Book.create(bookData); addedCount++; }
    }
